...
(function () {
  ...
  let createAnimationPage = function (updtFromHistory: boolean): DocumentFragment {
  ...
  let loadLoginPage = function(fromHistory: boolean = false): void {

    clearPage();
    if (!fromHistory) addToHistory("login");
    div.appendChild(createLoginPage());

  }

  let loadAnimationPage = function(fromHistory: boolean = false): void {

    clearPage();
    div.appendChild(createAnimationPage(fromHistory));
    if (!fromHistory) addToHistory("animation");
  ...
  let addToHistory = function (urlAdd: string): void {
